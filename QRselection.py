...
def qr_selection(con_list, models, vir_xpoint, vir_ypoint, y_quantile, gen_x_point):
    l = len(gen_x_point)
    while l > 0:
...
def qr(sample, vir_xpoint, vir_ypoint, models, checked_ylist, checked_xlist, dim, y_quantile):
    ...
    if q_index < len(y_quantile):
        ...
        if b > 0 and (closed_y_n - y_pre) * (closed_x_n - x) > 0 or b < 0 and (closed_y_n - y_pre) * (closed_x_n - x) < 0:
            sample.checked = 1
